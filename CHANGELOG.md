...
console 的 Request Log 页自己也是 "No requests"）。1.1.11.43 把这类缺口记成 skipped 后**不再重试**，
好处是不再每 5 分钟白跑，坏处是官方哪天把更早的日志补回来，我们不会自动捡起来。

**改动**：备忘录里带了 `at` 时间戳，**超过 24 小时就再跑一轮回填**（其余逻辑不变：查通了还是空 → 继续记 skipped）。
一天一次请求量可以忽略，但数据回来了会自动补齐。

**验证**：本机回填日志显示 `回填 18 天（…4 天超出 30 天保留期）` → `官方没有日志（18 天 + 超保留 4 天）→ 记为只能看总额`，
备忘录写入 `logic=request-logs-v1`（14:37 那次就是「备忘录变更 → 自动重跑 → 写入新备忘录」的完整往返）。

> 诚实说明：「24 小时后再试」这条本身只做了**代码级**核对（`Date().timeIntervalSince(lastAt) > 24h`）——
> 手改 plist 里的 `at` 会被 cfprefsd 缓存挡住（App 读到的仍是旧值），所以没能在真机上现场触发一次；
> 明天自然到期时的第一次重试就会走这条路径。

版本 **1.1.11.44 (84)**。

### v1.1.11.43 — 用量数据源迁移：`usage/rows` → `request-logs`（顺带修掉 305%）

**现象**：历史明细永远卡在「25/34 天」、今日模型出现 **305%**、今日金额不更新、按 Key 视图停更。

**根因（实测）**：上游把 `GET /console/api/usage/rows` 撤了 —— `range=30d/7d/24h/all`、带不带
cursor/since **全部 404**。而这个接口是"每天 × 每模型 × 每 Key"明细的唯一来源，于是：
1. 回填每 5 分钟重试、每 5 分钟空手而归（日志原文：`回填一条也没抓到（接口抖动）`）→ 进度条永远不动；
2. 卡片上"今日合计"来自这次抓取、"今日每模型明细"来自合并后的缓存 —— 两个来源各走各的，
   实测 `$0.112` vs `$0.346` → 界面按 明细/合计 画出 **305%**。

**修法（数据源整体换到 /logs 页面用的新接口）**：
- **明细来源**：`request-logs`（一条记录带 `serviceAPIKeyID / model / cost(美元) / startedAt(ms)`）。
  由 `UsageRows.rowFromLog()` 转成**和以前完全一样的行格式**，归日/拆分规则一行没改（仍是唯一真源）。
  抓取用 `export`（单次 ≤1000 条，`truncated` 就按时间中点**二分递归**）+ 增量用分页（100/页）；
  单次 60s 超时并重试一次。
- **每日总额兜底**：`cost-by-day&bucket=hour` 小时桶 → 按**北京时间**重分桶（老接口给的是 UTC 日，
  和我们的日界差 8 小时）。
- **对账**：每次刷新顺带比一次 `usage/models?since=今天0点`，差 >1% 记 warning（上游再换口径能立刻发现）。
- **保留期边界**：日志只保留 30 天，且实测 9/19 之前的窗口**一条都没有**（虽然总额还在）。
  这两类"补不了的天"写进 `historyRepairLast.skipped`，**不再重试**；进度条按"可补天数"算，
  补完显示"N 天官方没有请求日志，只能看总额"。
- **305% 从根上消掉**：快照里合计直接取"明细之和"（新增 `UsageMerge.totalMismatch` 护栏用于记日志），
  界面百分比钳到 0–100。

**实测（本机真机）**：
- 独立两算对齐：9/25 日志逐条 $0.278950 ↔ 小时桶 $0.2790（0%）；9/26 $0.861477 ↔ $0.859836（0.19%）。
- 按 Key 恢复：9/25 = 临时 `$0.227765` + 方泽恩 `$0.051185`；按模型 `deepseek-v4.1-flash $0.27895`。
- App 日志：`今日对账一致（官方 0.904193 ≈ 日志 0.900583）`；快照 `costTotal == entries 相加`（不再 305%）。
- 历史回填：25/34 → **28/30**（30 = 34 天窗口减去 4 天超出保留期；另有 5 天官方无日志已标记，不再重试）。
- 新增 `scripts/probe-usage-vs-logs.sh`：独立把"日志逐条"和"小时桶"各算一遍，逐日对比，run 一次全绿。
...
点「清除用量缓存」也不出现，只有丁雁 / 方泽恩两把老钥匙。

**根因（读代码 + 查本机缓存坐实）**：
1. 下拉框的密钥列表是 `cachedOrFetchedKeys()`：**有缓存就直接返回**，缓存只有在为空时才现拉 ——
   于是 9/19 之后新建/吊销的钥匙永远不会反映到界面。
2. 「清除用量缓存」的提示写着"会删掉本机存的…密钥列表"，可 `wipeUsageCache()` 只删了用量快照，
   `availableKeys` 一个字节没动 —— 提示和代码打架，用户怎么清都没用。

**修法**：
- **刷新就连带重拉密钥列表**：`cachedOrFetchedKeys(force:)`，App 每次「刷新」（含每 5 分钟自动那轮）传
  `forceKeys: true`；这次重拉与用量抓取**并发**跑，刷新总耗时不变。拉失败或结果为空 → 静默退回缓存，
  下拉框绝不清空。
- **过滤规则抽成纯函数** `ApiKeyInfo.parseConsoleKeys()`：跳过已吊销（`revokedAt` 非空）、非 `active`、
  以及 `expiresAt` 已过期的 key；账号名去掉接口里的 `Legacy: ` 前缀；`name` 为空用 id 兜底；保持接口顺序。
  以前这套判定埋在网络方法里，既没法离线测、也没规定过期怎么算。
- **清缓存名副其实**：`wipeUsageCache()` 一并删掉 `availableKeys`（换账号那条路径自动受益）。
- **日志**：每次刷新记一行 `密钥列表刷新：N 把有效（跳过 M 把已吊销/已过期）`，拉失败记 warning。

**验证**：
- 新增离线 fixture 测试 `Tests/KeyListParseTests.swift`（挂进 `scripts/test-key-parse.sh`，打包前必跑）：
  用真实结构 6 把 key（含已吊销 1、已过期 1、无名字 1）断言只留 4 把、顺序不变、`Legacy: ` 被去掉、
  无名字用 id 兜底、时间边界与解析失败不误删、畸形输入不崩。
- 真机：修前本机缓存只有 2 把 → 点「刷新」后下拉框出现 3 把（丁雁 / 方泽恩 / 临时），
  `清除用量缓存` 后密钥列表确实为空、再刷新重建。
