...
背景：Go 新上限时免费 `Union Alpha Free`（网关 id `union-alpha`，限时、不计配额）。三条链路同时挂：①小组件配额表解析把它整行丢了（三个配额格写「无限制」，白名单里只有「无限」）；②Codex 侧 `model_discovery.py` 同一个病根，`union-alpha-go` 进不了 `models.json`；③就算进去了也跑不通——直打实测 `union-alpha` 只在 `/v1/messages`（Anthropic 格式）返回 `200`，`/responses` 与 `/chat/completions` 恒 `500`，而同一时刻 `qwen3.8-max / minimax-m3 / glm-5.3 / omen-alpha / hy4-preview` 全部 `200`（排除网关整体故障）。
处置：
- 小组件（1.1.9.3）：`GoQuotaRegistry` 免费白名单加「无限制/不限/不限量/free/unlimited」，缓存键升 `v3`，slug 归一 `union-alpha-free → union-alpha`，兜底名单刷到 28 行（`ox-alpha-free` 让位）；`GoQuotaChart` 删掉写死的 ox 分支、脚注改成从数据取免费行名。
- 代理新增 Anthropic Messages 通道：`MESSAGES_ALWAYS_BRIDGE = {union-alpha}` 直接走桥（不再浪费必 500 的 `/responses` 探测），新增 `_responses_request_to_messages`（instructions→system、function_call→`tool_use`、function_call_output→`tool_result`、tool→`input_schema`、连续同角色合并、`max_tokens` 必填兜底 32000）、`MessagesBridgeTranslator`（`message_start/content_block_delta/message_delta/message_stop` → Responses SSE，复用 chat 桥记账与字节预算）、`_build_messages_fallback_json`（非流式）；headers 换成 `x-api-key` + `anthropic-version: 2023-06-01`（Bearer 会 `401 Missing API key`）。
- **`x-opencode-session` 是这次捡到的线上雷**：官方开始强制该头，缺了直接 `400 MissingSessionID`，且 chat 端点也中招——实测 chat 桥（glm/mimo/qwen）在 14:01 已因此 502。代理以前从不发这个头，现在 go/zen 路由统一按「instructions+前几条消息+模型」指纹补发（同一对话稳定、不同对话不串号）。
...
### 32. 续费换账期后「图是空的、数字还是上个月的钱」（2026-09-19，1.1.9.9）

现象：用户续费，套餐新周期 9/19 10:03 开始（`monthlyReset` 10/19）。账期标签页顶部显示 **$24.11**（上一个自然月的花费），下面柱子却是空的；切「自然月」也是 $24.11。
根因（两个叠加）：
1. **总数和柱子各算各的**：`App.swift` 顶部那个数是 `filteredDaily.reduce(0) { $0 + $1.total }` —— 把快照里**所有天**相加，跟账期/自然月开关无关；而柱子走 `MonthChartView.effectiveDates`（按窗口过滤）。窗口里没数据 → 图空；总数照旧把窗口外的天算进来 → $24.11。
2. **抓取器把「新周期还没用量」当成抓取失败**：`CostCrawler.fetchBillingCycleCosts` 先按账期窗口过滤，`if daily.isEmpty { return nil }`。新周期刚开、零用量 → nil → `WidgetSnapshotRefresher` 走「保旧」分支，把上一个月的数据当成本次结果继续用。反过来，一旦新周期有了用量（比如 9/19 用了 $0.04），窗口过滤只剩 9/19 一天，**9/1–9/17 的历史又被丢掉**，自然月视图跟着变空。
修复：
- 抽出 `Sources/ChartWindow.swift`：窗口唯一真源（账期 = 月重置日往前一月；自然月 = 数据锚点所在月），`dates/dateKeys/total` 三个入口；顶部总数改成 `ChartWindow.total(...)`，与柱子共用同一套日期。顺手把 `MonthChartView.windowDates/windowTotal` 的重复实现删掉。
- `fetchBillingCycleCosts` 不再只留账期窗口内的天：保留抓到的**整月**数据（月粒度抓取天然覆盖「当前账期 + 当前自然月」），由各视图自己按窗口过滤。这样账期看 9/19–10/18（$0.09），自然月看 9/1–9/30（$24.59），两边都完整；`if daily.isEmpty` 只在真的什么都没抓到（或整段无用量）时才返回 nil 走回退。
